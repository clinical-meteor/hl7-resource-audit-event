...
export default class AuditEventDetail extends React.Component {
  getMeteorData() {
    let data = {
      auditEventId: false,
  ...
  addToContinuityOfCareDoc(){
    console.log('addToContinuityOfCareDoc', Session.get('auditEventUpsert'));

    var auditEventUpsert = Session.get('auditEventUpsert');

    var newAuditEvent = {
      'resourceType': 'AuditEvent',
      'status': auditEventUpsert.status,
      'identifier': auditEventUpsert.identifier,
      'code': {
        'text': auditEventUpsert.code.text
      },
      'performedDateTime': auditEventUpsert.performedDateTime  
    }

    console.log('Lets write this to the profile... ', newAuditEvent);

    Meteor.users.update({_id: Meteor.userId()}, {$addToSet: {
      'profile.continuityOfCare.auditEvents': newAuditEvent
    ...
    if (auditEventId) {
      return (
        <div>
          ...
          <RaisedButton id="addAuditEventToContinuityCareDoc" label="Add to CCD" primary={true} onClick={this.addToContinuityOfCareDoc.bind(this)} style={{float: 'right'}} />
        </div>
      );
    } else {
      return(
